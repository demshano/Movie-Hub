...
import movieApp_logo_1 from '../images/movieApp_logo_1.png';
import  '../App.css';
export const Header = () => {

const activeClass="block py-2 pl-3 pr-4 text-white bg-orange-700 hover:text-xl rounded md:bg-transparent md:text-orange-700 md:p-0 md:dark:text-orange-500";
const inActiveClass = "block py-2 pl-3 pr-4 text-blue-900 rounded hover:text-emerald-400 hover:text-xl md:hover:bg-transparent md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700";
...
<nav className="bg-white border-b-2 border-gray-200 dark:bg-gray-900 px-2 sm:px-4 py-2">
  <div className=" flex flex-wrap items-center justify-between mx-auto p-4">
  <Link href="https://flowbite.com/" className="flex items-center" to='/'>
      <img src={movieApp_logo_1} className="h-8 mr-3" alt="movieMate" />
      <span className="self-center text-2xl font-semibold whitespace-nowrap text-black dark:text-white" >Movie Hub</span>
  </Link>

  <div id='mobile-nav' className="flex md:order-2">

  <button onClick={() => setDarkMode(!darkMode)} data-tooltip-target="navbar-search-example-toggle-dark-mode-tooltip" type="button" data-toggle-dark="light" className="flex items-center py-1 px-2 mr-2 text-xs font-medium text-gray-700 bg-white rounded-lg border border-gray-200 toggle-dark-state-example hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-500 dark:bg-gray-800 focus:outline-none dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
    ...
    <div className={`${hidden ? "hidden" : ""} items-center justify-between  w-full md:flex md:w-auto md:order-1`} id="navbar-search">
      ...
      <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
        <li>
          <NavLink to="/" className={({e})=> e ? activeClass : inActiveClass} aria-current="page" end>Home</NavLink>
        </li>
        <li>
          <NavLink to="/movies/popular" className={({e})=> e ? activeClass : inActiveClass} >Popular</NavLink>
        </li>
        <li>
          <NavLink to="/movies/top" className={({e})=> e ? activeClass : inActiveClass}>Top Rated</NavLink>
        </li>
        <li>
          <NavLink to="/movies/upcoming" className={({e})=> e ? activeClass : inActiveClass}>Upcoming</NavLink>
        </li>
      </ul>
